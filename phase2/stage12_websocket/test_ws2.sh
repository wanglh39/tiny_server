#!/bin/bash
# test_ws2.sh —— 只用 Python 测试（跳过 curl）
cd /mnt/c/Users/wlh19/Desktop/webserve

pkill -f ws_server 2>/dev/null || true
sleep 0.5

timeout 10 ./build/bin/ws_server 8080 &
SERVER_PID=$!
sleep 1.5

echo "服务器 PID=$SERVER_PID"
echo "检查进程: $(ps aux | grep ws_server | grep -v grep)"
...
print(f"握手响应: {response.split(chr(13))[0]}")
